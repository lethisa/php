...
$query = "SELECT customer.customer_hp, customer.customer_name, customer.customer_town, groups.groups_name, customer.customer_meja, customer.customer_kursi, groups.groups_visit, groups.groups_barcode FROM customer JOIN groups ON customer.customer_group = groups.groups_id WHERE customer_status = 1";
...
$spreadsheet->getActiveSheet()->getColumnDimension('H')->setAutoSize(true);
    ...
    while ($row = mysqli_fetch_assoc($result_groups)) {
        ...
        $activeSheet->setCellValue('G'.$i , $customer_meja)->getStyle('B'.$i)->getFont()->setBold(false);
        $activeSheet->setCellValue('H'.$i , $customer_kursi)->getStyle('B'.$i)->getFont()->setBold(false);
        $i++;
    }
